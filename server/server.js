...
app.get("/api/get-turn-credentials", (req, res) => {
  const accountSid = "AC973625cbd05ae31d2fec92c3b0b5f7b4";
  const authToken = "f21cbd529d1f731328352ba7ba0ab2f3";
...
const joinRoomHandler = (data, socket) => {
  ...
  // If the room doesn't exist, create a new room object
  if (!room) {
    room = {
      id: roomId,
      connectedUsers: [],
  ...
  // Update the connectedUsers property of the room
  room.connectedUsers = room.connectedUsers ? [...room.connectedUsers, newUser] : [newUser];
  ...
  room.connectedUsers.forEach((user) => {
    if (user.socketId !== socket.id) {
      ...
      console.log(data,socket.id,"111111111s");
...
const signalingHandler = (data, socket) => {
  ...
  console.log({ connUserSocketId, signal },"3333333")
  const signalingData = { signal, connUserSocketId:socket.id };
  console.log(signalingData,"44444444444")
